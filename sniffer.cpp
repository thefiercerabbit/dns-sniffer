 ...
 * the fly, filters DNS requests (UDP and destination port 53) and
 * writes the result into a tap interface.
 * 
 * The original file can be found here: https://github.com/mfontanini/dot11decrypt
 * It does not implements the DNS filter but does all the rest!
 */
...
public:
  ...
  void run() {
    thread_ = thread(&dns_sniffer::thread_proc,this);
  }
...
private:
  ...
  void thread_proc() {
    ...
    using std::placeholders::_1;
    sniffer.sniff_loop(bind(&dns_sniffer::callback, this, _1));

    //sniffer.sniff_loop(this->sniff_callback);
  }
